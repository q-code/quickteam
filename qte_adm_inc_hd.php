...
<a class="item'.($oVIP->selfurl==APP.'_adm_secu.php' ? ' actif' : '').'" href="'.APP.'_adm_secu.php" onclick="'.($oVIP->selfurl==APP.'_adm_secu.php' ? 'return false;' : $str).'">',$L['Adm_security'],'</a>
<a class="item'.($oVIP->selfurl==APP.'_adm_fields.php' ? ' actif' : '').'" href="'.APP.'_adm_fields.php" onclick="'.($oVIP->selfurl==APP.'_adm_fields.php' ? 'return false;' : $str).'">',$L['Field_man'],'</a>
</div>
<div class="group">
...
if ( count($arrModules)>0 )
{
  ...
  echo '<a class="item'.($oVIP->selfurl==APP.'m_'.$strKey.'_adm.php' ? ' actif' : '').'" href="'.APP.'m_'.$strKey.'_adm.php" onclick="return qtEdited(bEdited,e0);">',$strValue,'</a>',PHP_EOL;
  }
}
...
<div class="footer"><a id="exit" href="'.APP.'_index.php" target="_top" onclick="return qtEdited(bEdited,e0);">',L('Exit'),'</a></div>
</div>
';
}
